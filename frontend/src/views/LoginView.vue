    ...
    <div class="relative w-full h-full z-10 flex flex-col lg:flex-row">
      ...
      <div class="hidden lg:flex flex-1 flex-col justify-between p-12 xl:p-20 pointer-events-none">
        ...
        <div>
          <h2 class="text-5xl xl:text-7xl font-bold tracking-tighter text-[var(--text-main)] leading-tight">
            Gestão <br>
            <span class="text-[var(--color-vintage-rose)] hover:text-[var(--color-vintage-mint)] transition-colors duration-700">Preditiva</span> <br>
            Inteligente.
          </h2>
          <p class="mt-6 text-xl text-[var(--text-muted)] max-w-md font-medium leading-relaxed">
            Impulsione o futuro da sua indústria com predições precisas, análises em tempo real e prevenção de falhas através de IA.
          </p>
        </div>
      ...
      <div class="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div class="w-full max-w-md backdrop-blur-xl bg-[var(--bg-app)]/80 dark:bg-[var(--bg-card)]/80 p-8 sm:p-12 rounded-3xl shadow-2xl border border-[var(--border-color)] relative pointer-events-auto transition-transform duration-300 hover:-translate-y-1">
          ...
          <div class="mb-10">
            <h3 class="text-2xl font-bold text-[var(--text-main)] tracking-tight">Bem-vindo(a) de volta!</h3>
            <p class="text-[var(--text-muted)] mt-2 font-medium">Acesse seu painel para continuar.</p>
          </div>
          ...
          <form @submit.prevent="handleLogin" class="space-y-6">
            <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="transform -translate-y-4 opacity-0" enter-to-class="transform translate-y-0 opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="transform translate-y-0 opacity-100" leave-to-class="transform -translate-y-4 opacity-0">
              <div v-if="errorMessage" class="p-4 rounded-xl border border-[var(--color-vintage-rose)] bg-[var(--color-vintage-rose)]/10 flex items-start gap-3">
            ...
            <div class="space-y-1.5 border-b-2 border-transparent focus-within:border-[var(--color-vintage-mustard)] transition-colors">
              <div class="flex justify-between items-center">
                <label class="block text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Senha de Acesso</label>
                <a href="#" class="text-xs font-bold text-[var(--color-vintage-blue)] hover:text-[var(--text-main)] transition-colors">Esqueceu?</a>
              </div>
              <div class="relative flex items-center">
...
import { Activity, Mail, Lock, AlertCircle, Loader2, ArrowRight } from 'lucide-vue-next'
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (error.response && error.response.status === 401) {
      ...
      errorMessage.value = 'Erro interno. Verifique se o servidor backend está online.'
    }
  } finally {
...
onMounted(() => {
  ...
  function initParticles() {
    ...
    // Vintage aesthetic palette for particles
    const colors = ['#C69F9B', '#90A99E', '#DABA78'] 
